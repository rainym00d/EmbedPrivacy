...
def preprocess_logits_for_metrics(logits, labels):
    return torch.nn.functional.normalize(logits[1], p=2, dim=1)
...
def main():
    ...
    for dataset_name in dataset_list:
        path = os.path.join(args.dataset_save_dir, dataset_name)
        dataset_dict[dataset_name] = datasets.load_from_disk(path)["test"]
    ...
    for dataset_name, test_dataset in dataset_dict.items():
        ...
        # * sentence embedding
        text_embeddings = np.array(test_dataset["embedding"])

        # * test each attribute
        # * get all attribute in test dataset
        attribute_list = []
        for k in test_dataset.features.keys():
            if k not in ["text", "embedding"] and "candidate_list" not in k:
        ...
        for attr in attribute_list:
            ...
            if trainer.is_world_process_zero():
                ...
                for i in range(sample_size):
                    ...
                    answer = scores.argmax(axis=-1)
                    if answer == 0:
                        metric += 1
                metric = metric / sample_size
                all_results[f"{dataset_name}_{attr}"] = metric

    if trainer.is_world_process_zero():
        output_file = os.path.join(args.output_dir, "attr_results_same_embedding_model.json")
